...
def load(commandclass, conn, **kwargs):
    if commandclass is None:
        return None
    kwargs['icpe'] = commandclass.sensor.icpe.mac_address
    kwargs['sensor'] = commandclass.sensor.sensor_id
    ...
    conn.hmset(commandclass.sensor.icpe.mac_address +\
               commandclass.sensor.sensor_id + kwargs['name'], kwargs)
    return kwargs

@redisconn
def save(mac_address, sensor_id, name, conn, **kwargs):
    field  = conn.hgetall(mac_address + sensor_id + name)
    ...
    return conn.hmset(mac_address + sensor_id + name, field)
